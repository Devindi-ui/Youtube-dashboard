const API_KEY = 'AIzaSyCROqnDTFRLqv5TnjvuaXmkSCCakFLP010';
...
async function searchChannel(channelName){
    try {
        const searchUrl = `${BASE_URL}search?part=snippet&type=channel&q=${encodeURIComponent(channelName)}&key=${API_KEY}`;
    ...
    const channelUrl = `${BASE_URL}channels?part=snippet,statistics,contentDetails&id=${channelId}&key=${API_KEY}`;
